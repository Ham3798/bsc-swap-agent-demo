...
- optional OpenAI API for intent extraction
- dry-run only execution by default

Current implementation reality:

- `bnbchain-mcp` provides core chain capabilities
- quote, payload, simulation fallback, and submission-path reasoning are implemented as demo-local planning extensions
...
If `OPENAI_API_KEY` is not set, the skill falls back to a local rule-based intent parser.
